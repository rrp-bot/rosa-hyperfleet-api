...
var _ = Describe("Platform API", Ordered, func() {
	...
	It("should have maestro-server connected to maestro-agent", func() {
		...
		By("waiting for the resource bundles to be created and have conditions set correctly", func() {
			...
			}, "10m", "5s").Should(BeTrue(), "No resource bundles found with Applied, Available, or StatusFeedbackSynced conditions - maestro-server may not be connected to maestro-agent")
		})
	})
